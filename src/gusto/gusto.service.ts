...
export class GustoService {
  ...
  async getOauthToken(userId: ModelId): Promise<OAuthToken | undefined> {
    return await this.oauthService.getOrRefreshSavedAccessToken(
      userId,
      OAuthProvider.GUSTO,
    )
  }
  ...
  async get(path: string, token: string) {
    ...
    const bearerToken = await this.getWithBearerToken(url, token)
