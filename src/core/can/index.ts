...
export class Can {
  ...
  static checkRequiresAuthentication<T>(
    target: Type<T>,
    ...actions: Array<ActionScope>
  ): boolean {
    const permissions = this.global.getRegisteredPermissions(target)
    ...
    return !every(
      actions.map(action => {
        return permissions.filter(
          p =>
            p.userScope === UserScope.Anyone && p.actions.indexOf(action) >= 0,
        ).length
      }),
    )
  }
